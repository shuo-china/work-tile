<template>
  <div>
    ...
    >
      <ProTableColumn prop="task_name" label="标题" min-width="400" fixed="left">
        <template #default="{ row }">
          ...
          <div v-else class="group absolute inset-0 flex flex items-center px-4">
            <div class="flex-grow-1">{{ row.task_name }}</div>
      ...
      <ProTableColumn prop="current_status" label="状态" width="140">
        <template #default="{ row }">
          <div
            :class="[
          ...
          >
            <StatusButton
              ...
              :data="taskStatusMappings[row.current_status]"
            />
      ...
      <ProTableColumn prop="level_status" label="优先级" width="140">
        <template #default="{ row }">
          <div
            :class="[
              'group flex justify-between table-col-clickable',
      ...
      <ProTableColumn prop="end_time" label="截止日期" width="160">
        <template #default="{ row }">
          <div
            :class="[
              'group flex justify-between table-col-clickable',
    ...
    <DatePickerPopover
      ...
      @update:disappear="
        (v: boolean) => {
          endTimePopoverDisappear = v
          if (v) {
            endTimePopoverDisappear = currentEndTimePopoverRef
          }
        }
      "
    />
...
import { getTaskPaginationApi, TaskItem, updateTaskApi } from '@/api/task'
